...
::: {.class style="color: firebrick"}
To show that the medicine group has a higher health score than the placebo group, I would use one-tailed t-test.
...
# group_medi <- runif(10)
group_medi <- c(0.985, 0.338, 0.624, 0.397, 0.658, 0.670, 0.895, 0.656, 0.460, 0.548)

# group_placebo <- runif(10)
group_placebo <- c(0.457, 0.577, 0.137, 0.180, 0.500, 0.387, 0.083, 0.172, 0.428, 0.595)
# mean(group_medi) - mean(group_placebo)
# 0.27

medi_data <- data.frame(group_medi, group_placebo)

t_medi <- t.test(group_medi, group_placebo, data = medi_data, alternative = 'greater')
...
```{r scatter_plot}
# # library(tidyverse)
# mean_medi <- sample(group_medi, size = 10, replace = TRUE)
# mean_placebo <- sample(group_placebo, size = 10, replace = TRUE)
# 
# diff_mean <- mean_medi - mean_placebo
# 
# ggplot(medi_data,
#        aes(x = diff_mean))+
#   geom_histogram()
