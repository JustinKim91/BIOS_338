...
#' 
#' @description 
#' `get_mean` returns the artimetic mean of its vector argument
#' 
#' @details 
#' This function takes the sum of all elements within the argument vector and find the length of the vector. It then
#' divides the sum of the elements by the length of the vector.
#' 
#' @param `x` is a numeric vector
#' 
#' @return A function that takes a numeric vector as an argument and returns the arithmetic mean of the vector
#' 
#' @example 
#' get_mean(1:5)
#'
...
::: {.class style="color: firebrick;"}
Reference: Class5_demo_loops-functions.R
:::
...
::: {.class style="color: firebrick;"}
The hiding of calculation results in R is a practice of object-orientated programming. The benefit object-oriented programming, specifically assignments in our case, is that it allows successive writing and maintenance of the code to be much easier and cleaner. For instance, if we are working with a variable with an assignment of a arithmetic operation, it is easier to work with that variable than it is to with the calculated value, especially if the arguments for the arithmetic operation themselves were variables.
:::

```{r assignment}
# Example

x <- 2 # Assign values to x and y
y <- 3
z <- x + y # Assign the operation of x + y to Z; this way, if the values of x and y change, we can just as easily work
# ...        work with z as before the variables were changed.

z # 5

# Reference: https://cran.r-project.org/doc/manuals/r-release/R-lang.html#Object_002doriented-programming
```
...
2.  The final plot uses a box plot as opposed to a bar graph
...
```{r improved-plot}
# improved plot
sleep_order <- sleeping_data %>% group_by(order) %>% mutate(count = n()) # Create new dataset that counts the number of individuals in each order

sleep_order_plot <- ggplot(sleep_order, aes(x = sleep_total, y = order)) + # Swapped axis so labels are no longer tilted
  geom_boxplot()+ # Replacing the bar graph with box and whisker 
  geom_point(position = "jitter", alpha = 0.5) + # Overlaying points on the boxplot with "small random variation" in location; also decreasing the opacity of the points.
  labs(x = "Sleep duration per day (hours)", y = "", title = "Mammals sleep duration changes across orders with a wide variation", subtitle = "Number of members in each order specified next to the y-axis") + # Labeling
  theme(plot.title = element_text(hjust = -2), plot.subtitle = element_text(hjust = -0.5), axis.line = element_line(colour = "black")) + # Moving title/subtitle to centralize them for better readibility
  geom_text(data = sleep_order %>% distinct(order, .keep_all = TRUE), # Adding count for the number of individuals in each order
            aes(y = order, x = 0, label = count),
            hjust = 1,
            size = 4,
            angle = 0)
  sleep_order_plot
  
  # References: https://ggplot2.tidyverse.org/reference/geom_jitter.html
...
```{r improved-plot-reordered}
# Recreating dataset so that the order is displayed in descending order of count
sleep_arranged <- arrange(sleep_order, desc(count))

# Mostly the same coding for original improved plot, but specify to graph box plots in order that they apear in dataset (arranged by
#... count)
ggplot(sleep_arranged, aes(x = sleep_total, y = fct_inorder(order))) + # Swapped axis so labels are no longer tilted
  geom_boxplot()+ # Replacing the bar graph with box and whisker 
  geom_point(position = "jitter", alpha = 0.5) + # Overlaying points on the boxplot with "small random variation" in location; also decreasing the opacity of the points.
  labs(x = "Sleep duration per day (hours)", y = "", title = "Mammals sleep duration changes across orders with a wide variation", subtitle = "Number of members in each order specified next to the y-axis") + # Labeling
  theme(plot.title = element_text(hjust = -2), plot.subtitle = element_text(hjust = -0.5), axis.line = element_line(colour = "black")) + # Moving title/subtitle to centralize them for better readibility
...
# Reference: https://stackoverflow.com/questions/4260698/r-ordering-in-boxplot
```
...
```{r improved-plot-title-formatting}
new_title <- expression(paste("Mammals", bolditalic(" sleep duration"), " changes across", bolditalic(" order"), " with a wide variation")) # Write formatting for new title so I can simply plug it in as an argument for my labs()


ggplot(sleep_order, aes(x = sleep_total, y = order)) + # Swapped axis so labels are no longer tilted
  geom_boxplot()+ # Replacing the bar graph with box and whisker 
  geom_point(position = "jitter", alpha = 0.5) + # Overlaying points on the boxplot with "small random variation" in location; also decreasing the opacity of the points.
  labs(x = "Sleep duration per day (hours)", y = "", title = new_title, subtitle = "Number of members in each order specified next to the y-axis") + # Labeling
  theme(plot.title = element_text(hjust = -2), plot.subtitle = element_text(hjust = -0.5), axis.line = element_line(colour = "black")) + # Moving title/subtitle to centralize them for better readibility
  geom_text(data = sleep_order %>% distinct(order, .keep_all = TRUE), # Adding count for the number of individuals in each order
            aes(y = order, x = 0, label = count),
            hjust = 1,
...
# Reference: https://stackoverflow.com/questions/67504377/how-to-use-bold-italic-and-underline-in-ggplot2
```
...
-   Write a function that takes two or more numbers and adds them, called `plus`. [(*1 point*)]{style="color: red;"}
    ...
    ```{r plus}
    # Naming a custom function "plus" that will take two inputs and add those two numeric values together.
...
-   Modify this function now such that it can also take `characters` (instead of numbers), and combines them using the `cat()` function. [(*2 points*)]{style="color: red;"}
    ...
    ```{r plus-modified}
    #
    plus_mod <- function(...){ # Modifying function so that if all of the arguments are numerical values,
      x <- c(...)
      if (class(x)=="numeric"){
        print(sum(x)) # the function will print the sum of the arguments
      } else { # Otherwise, the function will combine the arguments into a character string
        return(capture.output(cat(x, sep = "")))
      }
    }
    ...
    # References: https://www.rdocumentation.org/packages/base/versions/3.6.2/topics/cat
...
vector_plus <- function(...) {
  ...
  for (i in 1:length(a[[1]])) {
    ...
    vec_out <- append(vec_out, plus(v))
  }
  return(vec_out)
}

vector_plus(1:3, c("a", "b", "c")) # "1a" "2b" "3c"
vector_plus(1:3, 4:6) # "14" "25" "36"
vector_plus(c(1,4,5), c(4,5,8), c(5,9,3)) # 10 18 16
vector_plus(c(1,4,5), c(4,5,8), c("x", "y", "z")) # "14x" "45y" "58z"
```
...
vector_plus <- function(...) {
  a <- list(...)
  if (!all(sapply(a, length) == length(a[[1]]))) {# sapply takes the length of all elements in a to compare the length of the input vectors.
...
vector_plus(1:2, 1:3) # Displayed message: "Error in vector_plus(1:2, 1:3) : Vector lengths do not match"

# Reference: https://www.guru99.com/r-apply-sapply-tapply.html
```
...
```{r vectorize_anything}
for (i in colnames(sleeping_data)) { # Iterate the for loop across the columns of sleeping_data
  data_type <- class(sleeping_data[[i]]) # Find the class type of each column
  print(paste(i, "-", data_type)) # data_type is a numeric, so use paste to convert to a character string; add hyphen 
}                                 # ... to display data type next to the name of the column
...
sleep_col_data <- map(sleeping_data, class) # Returns list of the data type for each colum in sleeping_data
sleep_col_data

sleep_col_data_vec <- map_vec(sleeping_data, class) # Returns classes as a character vector
sleep_col_data_vec

# Reference: https://purrr.tidyverse.org/reference/map.html
# These functions are part of the purrr package in Tidyverse
```
...
```{r plotting-multiple-columns}
# plotting here
df <- tibble(x = 1:10, y1 = x, y2 = x^2, y3 = x^3)
ggplot(df, aes(x = x))+
  geom_smooth(aes(y = y1), colour = "dodgerblue") +
  geom_smooth(aes(y = y2), colour = "deeppink") + 
  geom_smooth(aes(y = y3), colour = "seagreen")
```

-   Use `pivot_longer()` to reshape the data and make a similar `ggplot` with different colours [(2+*1 points*)]{style="color: red;"} (learn how to use the `pivot_wider()` function from documentation and cite your sources)

```{r pivot-longer}
df_pivot <- pivot_longer(df, cols = c(y1, y2, y3), names_to = "Powers", values_to = "Value")

ggplot(df_pivot, aes(x = x, y = Value, color = Powers)) +
  geom_smooth()
...
### 8.b.bonus. Why is `pivot_longer()` better here?

(bonus) Discuss how this `pivot_longer` approach would be helpful compared to the previous approach [(*1 point*)]{style="color: red;"}

::: {.class style="color: firebrick;"}
For the pivot_longer, we do not have to code each plot individually using geom_smooth. Rather, pivot_longer condenses the columns by moving all y1, y2, and y3 values into the same column titled "Powers". Then, the pivot_longer function differentiates the entries under the "Powers" column and plots these values as three different plots on the same graph.
...
::: {.class style="color: firebrick"}
I would like to acknowledge various sources such as StackOverflow forums, RDocumentation, ggplot Documentation, and all of the linked resources provided within the assignment, Furthermore, I would like to acknowledge my groupmates Seungheon Lee and Gary Shi for our collaboration throughout the assignment.
:::
